...
const DIM_LABELS: Record<string, string> = {
  cof_coverage: 'COF',
  discovery_quality: 'SPIN',
  argument_coherence: 'SALES',
  objection_handling: 'Challenger',
...
function DebriefContent() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data: { session } }) => {
      ...
      try {
        ...
        setError(err instanceof Error ? err.message : 'Could not load debrief')
      } finally {
        setLoading(false)
      }
    })
  ...
  if (error || !debrief) {
    return (
      <div className="min-h-screen bg-[#10141a] flex flex-col items-center justify-center gap-3 p-6">
        ...
        {error && <p className="text-[11px] text-[#5f6368] font-mono">{error}</p>}
  ...
  // Framework dimensions — pick 4 canonical ones in order
  ...
  return (
    <div className="min-h-screen bg-[#10141a] flex flex-col">
      ...
      <main className="flex-1 p-4 max-w-2xl mx-auto w-full space-y-4">
        ...
        {/* 4. Coaching Notes per framework */}
                    ...
                    <tbody>
                      {(debrief.rag_claims ?? []).map((claim, i) => (
                        <tr key={i} className="border-b border-white/[0.04] last:border-0">
                          <td className="py-2 pr-3 text-[#9aa0a6] max-w-[200px] truncate">
                            {/* textContent rendering — never innerHTML */}
                            <span title={claim.excerpt}>{claim.excerpt}</span>
                          </td>
        ...
        <div className="flex flex-col items-center gap-3 pt-2 pb-6">
          <a
            href="/session/new"
            className="text-sm text-[#9aa0a6] hover:text-[#2ddbde] transition-colors"
          >
