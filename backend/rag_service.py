...
async def retrieve(
    ...
    implemented as a parameterized JOIN — never a prompt instruction.
    ...
    if session_mode == "certification":
        # Certification mode: INNER JOIN rag_manifest; filter on both approved flags.
        # Chunks with NULL manifest_id (pre-Phase 1 admin library) are excluded.
        sql = text("""
            SELECT
                kc.id,
                kc.content,
        ...
        # Practice mode: all active chunks; LEFT JOIN to surface manifest metadata
        sql = text("""
            SELECT
                kc.id,
                kc.content,
    ...
    # Audit log: write every retrieval to rag_retrievals (non-fatal)
