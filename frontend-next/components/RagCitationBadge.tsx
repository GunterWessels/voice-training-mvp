...
export function RagCitationBadge({ citation }: Props) {
  ...
  const label = `[${citation.source_doc.length > 18 ? citation.source_doc.slice(0, 16) + '…' : citation.source_doc}${citation.page != null ? ` p.${citation.page}` : ''}]`

  // Escape for display — use textContent semantics via React (never innerHTML)
  const escapedFullName = citation.source_doc.replace(/[<>"'&]/g, c =>
    ({ '<': '&lt;', '>': '&gt;', '"': '&quot;', "'": '&#39;', '&': '&amp;' }[c] ?? c)
  )
  ...
  return (
    <span className="relative inline-block">
      ...
      {showTooltip && (
        ...
        >
          <span className="text-[#e8eaed]">{escapedFullName}</span>
